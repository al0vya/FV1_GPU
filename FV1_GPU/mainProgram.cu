...
__global__ void fluxHLL(FaceValues d_faceValues, StarValues d_starValues, Fluxes d_fluxes)
{
	...
	if (x < d_simulationParameters.cells + 1)
	{
		...
		else
		{
			...
			hStar = pow(((aL + aR) / 2 + (uWest - uEast) / 4), 2) / d_solverParameters.g;
			...
			momentumFL = uWest * d_starValues.qWestStar[x] + d_solverParameters.g / 2 * pow(d_starValues.hWestStar[x], 2);
			momentumFR = uEast * d_starValues.qEastStar[x] + d_solverParameters.g / 2 * pow(d_starValues.hEastStar[x], 2);
